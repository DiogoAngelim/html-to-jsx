...
export function cssToObject(cssString: string): string {
    ...
    const styles = cleanCss.split(';')
        ...
        .map((style: string) => {
            ...
            if (!property || !value)
                return '';
            const camelProperty = property.replace(/-([a-z])/g, (_: string, letter: string) => letter.toUpperCase());
...
export const eventAttributesCallback = (_match: string, eventName: string, handler: string): string => {
    let newEventName = '';
...
export function closeSelfClosingTags(html: string): string {
    const result = html.replaceAll(new RegExp(`<(${selfClosingTags.join("|")})(?=[\s>/])([^>]*)\s*/?>`, "gi"), (_match: string, tagName: string, attributes: string) => `<${tagName}${attributes ? attributes : ""}/>`);
...
export const isTagClosed = (tag: string): boolean => {
    return !selfClosingTags.includes(tag) && tagsRequiringClosing.has(tag);
};

export const validateInput = (html: string): void => {
...
export const validateTags = (html: string): void => {
...
export function toCamelCase(str: string): string {
    return str
        .split(/[-_\s]/)
        .map((word: string, index: number) => index === 0
            ? word.toLowerCase()
